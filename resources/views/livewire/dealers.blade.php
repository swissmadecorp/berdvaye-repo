...
@stop
        ...
        <div class="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4 bg-white dark:bg-gray-900 p-2">
                <!-- <form action="customers/create"> -->
                    @role('superadmin|administrator')
                        <button wire:click.prevent="createNewDealer()" class="editdealer bg-sky-500 hover:bg-[#0284c7] text-white font-bold text-sm px-3 py-1.5 rounded">Create New</button>
                    @endrole
                <!-- </form> -->
            ...
            <livewire:dealer-item />

            <table class="w-full text-sm text-left rtl:text-right dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        <th scope="col" class="px-3 py-3">Company</th>
                        <th scope="col" class="px-3 py-3">Date Created</th>
                        <th scope="col" class="px-3 py-3"></th>
                    </tr>
                </head>
                <tbody>
                    @foreach($dealers as $dealer)
                        <tr wire:key="{{$dealer->id}}"
   ...
   {{ $dealers->links('livewire.pagination') }}
   </div>
