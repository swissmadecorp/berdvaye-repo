...
@stop
    ...
    <div x-data wire:ignore.self id="slideover-order-container" class="fixed inset-0 w-full h-full invisible z-50">
        ...
        <div @keydown.escape.prevent="closeAndClearFields()"  wire:ignore.self id="slideover-order" class="absolute duration-500 ease-out transition-all h-full bg-white right-0 top-0 translate-x-full overflow-y-scroll w-full" >
            ...
            <div id="slideover-order-child" class="p-2 flex items-center shadow-sm rounded-full justify-center hover:bg-gray-300 cursor-pointer absolute top-0 right-0 m-2">
            ...
            <div id="default-styled-tab-content" class="dark:bg-gray-900">
                <div wire:loading.delay.long class="fixed z-50">
                    <div class="text-center fixed left-0 top-0 bg-black bg-opacity-50 w-screen h-screen justify-center items-center z-50">
                ...
                <div wire:ignore.self x-data="{flex: 1}" class="p-4 rounded-lg dark:bg-gray-800" id="customer-info" role="tabpanel" aria-labelledby="customer-info-tab">
                    <form>
                            ...
                            <table wire:ignore.self class="w-full text-sm text-left rtl:text-right dark:text-gray-400">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3 w-16">Reference</th>
                                ...
                                @foreach ($items as $index => $item)
                                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                        <th class="text-center" wire:model="item.linenumber">
                                        @if ($item['id'])
                                            {{$index+1}}
                                        @endif
                                        </th>
                                        ...
                                        <td class="px-4 py-4"><input class="w-16 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" wire:model="items.{{$index}}.p_model" type="text"></td>
                                        ...
                                        <td class="px-4 py-4">
                                            <input wire:model.live="items.{{$index}}.price" data-id="{{ $item['op_id'] }}" data-name="price" id="price-{{ $index }}" type="text" style="width: 80px" placeholder="0.00" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" pattern="^\d*(\.\d{0,2})?$">
                                            @if ($items[$index]['op_id'])
                                        ...
                                        @if ($transferToOrder)
                                        <td class="px-4 py-4 text-right"><input data-linenumber="{{ $index + 1 }}" class="serial-input bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" wire:model="items.{{$index}}.serial" type="text"></td>
                                        @endif
                                        <td class="px-4 py-4 text-right">{{ $item['cost'] }}</td>
                                ...
                                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-4"><input type="text" placeholder="Reference" wire:model.lazy="newProductId" id="reference_name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" type="text" /></td>
                                    <td colspan="2" class="px-4 py-4"></td>
                                </tr>
                                </tbody>
                                <tfoot class="bg-gray-50 dark:bg-gray-700 dark:text-gray-400 text-gray-700 text-xs uppercase">
                                    <th class="p-4" colspan="7">Items Total</th>
                        ...
                        @if ($orderId)
                        <div>
                            <button wire:ignore wire:click="saveOrder()" id="update" type="button" class="text-white mt-4 bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Update</button>
                            <button wire:ignore wire:click="TransferToOrder()" id="makeinvoice" type="button" class="text-white mt-4 bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">Make invoice</button>
...
@script
    <script>
        $(function() {
            ...
            $wire.on('scroll-to-top', msg => {
                $('#slideover-order').animate({
                    scrollTop: 0
                }, 100);
            ...
            $(document).on('focus', '.serial-input', function () {
                if($(this).devbridgeAutocomplete() === undefined ){
                    $('.serial-input').devbridgeAutocomplete({
                        ...
                        zIndex: 900,
                        onSelect: function (suggestion) {
                            let lineNumber = $(this).data('linenumber');
                            $wire.$call('editProductRow', lineNumber, suggestion)
                            // $wire.$set('items.s_firstname',suggestion.value);
                        }
                    });
                }
            ...
            $('#reference_name').devbridgeAutocomplete({
                serviceUrl: "{{route('find.product')}}",
                showNoSuggestionNotice: true,
                width: 250,
                minChars: 2,
                zIndex: 900,
                onSelect: function(suggestion) {
                    $wire.$call('newProductRow', suggestion);
                }
            });
