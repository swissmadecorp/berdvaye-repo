        ...
        >
            <div @click="dropdownOpen = false" class="rounded-md bg-gray-200 dark:bg-gray-900 divide-y divide-gray-100 shadow-xs">
                <div class="py-1" role="none">
                    <?php
                        if (! function_exists('loc')) {
                            function loc($id,$l,$isOrderPage=false) {
                                ...
                                else $window = "window.open('/admin/orders/print/$id'";
                    ...
                    ?>
                    <a href="#" onclick="<?= loc($id,'print',$isOrderPage)?>" class="{{$class}}">Print Invoice</a>
                    @if ($isOrderPage==false)
                ...
                @endif
                <div class="py-1" role="none">
                    <a href="#" wire:click.prevent="deleteInvoice({{$id}})" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white dark:text-gray-200">Delete Order</a>
                </div>

                @if (isset($transaction_id) && $transaction_id)
                <div class="py-1" role="none">
                    <a href="#" wire:click.prevent="refundInvoice({{$id}})" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white dark:text-gray-200">Refund Order</a>
                </div>
                @endif
            </div>
        </div>
