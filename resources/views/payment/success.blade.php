...
@section ('content')
    ...
    <div style="background: #fff; padding: 25px;border-radius: 4px;">
        <div class="container">
                ...
                <table class="table table-bordered table-sm">
                    ...
                    foreach ($response as $key => $product) {
                        if (isset($response['L_AMT'.$i])) {
                            ...
                            if ($response['L_NAME'.$i]=="Discount") {
                                ...
                                echo "<td style='color: red;' class='text-right'>$".number_format($response['L_AMT'.$i],2)."</td>";
                                echo "<td style='color: red;' class='text-right'>$".number_format($total,2)."</td>";
                            } else {
                                echo "<tr><td>".$response['L_NAME'.$i]."</td>";
                                echo "<td class='text-center'>".$response['L_QTY'.$i]."</td>";
                                echo "<td class='text-right'>$".number_format($response['L_AMT'.$i],2)."</td>";
                    ...
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <td class='text-right'>${{number_format($response['SHIPPINGAMT'],2)}}</td>
                        </tr>
                        @if ($response['TAXAMT']>0)
                        <tr>
                            <td colspan="3" class="font-weight-bold text-right">Tax (NYS):</td>
                            <td class='text-right'>${{number_format($response['TAXAMT'],2)}}</td>
                        </tr>
                        @endif
                        <tr>
                            <td colspan="3" class="font-weight-bold text-right">Total Amount:</td>
                            <td class='text-right'>${{number_format($response['AMT']+$response['TAXAMT']+$response['SHIPPINGAMT'],2)}}</td>
                        </tr>
                    </tfoot>
                </table>
            ...
            {{  Form::open(array('action'=>array('PayPalController@Checkout','token'=>$response['TOKEN']))) }} 
