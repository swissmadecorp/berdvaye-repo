...
class Orders extends Component
{
    ...
    public function sendEmail($ids) {
        ...
        LivewireAlert::title('Successfully emailed invoice!')->success()->position(Position::TopEnd)->toast()->show();
    }

    public function deleteInvoiceById($data) {
        $id = $data['id'];
    ...
    public function deleteInvoice($id) {
        LivewireAlert::title('')
            ->text('Are you sure you want to remove order#' . $id .'?')
            ->asConfirm()
            ->withOptions([
                'background' => '#f0f0f0',
                'customClass' => [
            ...
            ->onConfirm('deleteInvoiceById', ['id' => $id])
            ->show();
    }
