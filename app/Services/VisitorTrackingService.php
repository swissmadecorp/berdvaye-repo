...
class VisitorTrackingService
{
    ...
    private function applySearchTermToQuery(Builder $query, string $searchTerm = ''): Builder
    {
        ...
        foreach (preg_split('/\s+/', trim($searchTerm)) as $word) {
            $query->where(function (Builder $wordQuery) use ($columns, $word) {
                foreach ($columns as $column) {
                    $wordQuery->orWhere($column, 'like', '%' . $word . '%');
                }

                $wordQuery->orWhereHas('profile', function (Builder $profileQuery) use ($word) {
                    $profileQuery
                        ->where('display_name', 'like', '%' . $word . '%')
